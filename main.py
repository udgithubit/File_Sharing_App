...
def send_verification_email(email: str) -> str:
    ...
    print(f"DEBUG - Generated token for {email}: {token}")  # Add this line
...
async def signup(user: User):
    ...
    user_dict["verified"] = False  # Changed from True to False to actually require verification
    del user_dict["password"]

    await users_collection.insert_one(user_dict)

    # Generate and print verification token
     # In send_verification_email()
    verification_url = send_verification_email(user.email)  # Changed to 8001
    print(f"Generated verification URL: {verification_url}")  # Debu

    return {
        "message": "User created successfully. Please check your email for verification.",
        "verification_url": verification_url  # Add this to the response
    }

@app.get("/verify-email/{token}")
async def verify_email(token: str):
    try:
        print(f"DEBUG - Received token: {token}")  # Add this line
        email = serializer.loads(token, salt="email-verify", max_age=86400)
        print(f"DEBUG - Decoded email: {email}")  # Add this line
        user = await get_user(email)
        if not user:
            raise HTTPException(status_code=400, detail="Invalid email")
        result = await users_collection.update_one(
            {"email": email, "verified": {"$in": [False, None]}},
            {"$set": {"verified": True}}
        )
        ...
        print(f"DEBUG - Verification error: {str(e)}")  # Add this line
...
async def serve_file(token: str, current_user: UserInDB = Depends(get_current_user)):
    ...
    # ... [all your existing routes like /signup, /login, etc.] ...

    # ===== DEBUG ROUTES =====


@app.get("/debug-generate-token/{email}")
async def debug_generate_token(email: str):
    """Temporary debug route to generate verification tokens manually"""
    token = serializer.dumps(email, salt="email-verify")
    return {
        "email": email,
        "token": token,
...
# Add this at the VERY END of your file (after all routes/functions)
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=8000)